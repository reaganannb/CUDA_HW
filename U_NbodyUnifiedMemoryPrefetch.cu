...
int NTotal; //rounded up total bodies
int NumberOfGpus;

float3 *P, *V, *F; //positions, velocities, forces
float *M; //masses
...
void setup()
{
	...
	//rounding up for same chunk size 
	NPerGPU = (N + NumberOfGpus - 1) / NumberOfGpus;
    NTotal  = NPerGPU * NumberOfGpus;   // >= N
	...
	cudaMallocManaged(&M, NTotal * sizeof(float));
    cudaErrorCheck(__FILE__, __LINE__);
    cudaMallocManaged(&P, NTotal * sizeof(float3));
    cudaErrorCheck(__FILE__, __LINE__);
    cudaMallocManaged(&V, NTotal * sizeof(float3));
    cudaErrorCheck(__FILE__, __LINE__);
    cudaMallocManaged(&F, NTotal * sizeof(float3));
    cudaErrorCheck(__FILE__, __LINE__);
	...
	//prefetching initial data to gpu0
	cudaMemPrefetchAsync(P, NTotal * sizeof(float3), 0);
    cudaMemPrefetchAsync(V, NTotal * sizeof(float3), 0);
    cudaMemPrefetchAsync(F, NTotal * sizeof(float3), 0);
    cudaMemPrefetchAsync(M, NTotal * sizeof(float),  0);
    cudaDeviceSynchronize();
    cudaErrorCheck(__FILE__, __LINE__);
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
		//prefetching gpus chunk
        for(int dev = 0; dev < NumberOfGpus; dev++)
        {
            int offset = dev * NPerGPU;
            size_t chunkSize3 = NPerGPU * sizeof(float3);
            size_t chunkSize1 = NPerGPU * sizeof(float);

            cudaMemPrefetchAsync(P + offset, chunkSize3, dev);
            cudaMemPrefetchAsync(V + offset, chunkSize3, dev);
            cudaMemPrefetchAsync(F + offset, chunkSize3, dev);
            cudaMemPrefetchAsync(M + offset, chunkSize1, dev);
        }
		...
		//dont need to copy memory since using unified mem
